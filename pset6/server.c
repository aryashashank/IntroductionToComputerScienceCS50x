...
char* indexes(const char* path)
{
    char* fullpathHTML = malloc(strlen(path)+11);
    char* fullpathPHP = malloc(strlen(path)+11);
    strcpy(fullpathHTML,path);
    strcpy(fullpathPHP,path);
    strcat(fullpathHTML, "index.html\0");
    strcat(fullpathPHP, "index.php\0");
     if (access(fullpathHTML, F_OK) == 0)
    {
        free(fullpathPHP);
        return fullpathHTML;
    }
    if (access(fullpathPHP, F_OK) == 0)
    {
        free(fullpathHTML);
        return fullpathPHP;
    }
...
while (feof(file) == 0)
{
    ...
    *length = *length + fread(buffer + size, 1, BYTES, file);
    size = size + 512;
}
*content = &buffer[0];
return true;
...
const char* lookup(const char* path)
{
    ...
    if(strcasecmp(".css", value)==0){
       
        return "text/css";
    }
    else if(strcasecmp(".html", value)==0){
        return "text/html";
    }
    else if(strcasecmp(".gif", value)==0){
        return "image/gif";
    }
    else if(strcasecmp(".ico", value)==0){
        return "image/x-icon";
    }
    else if(strcasecmp(".jpg", value)==0){
        return "image/jpeg";
    }
    else if(strcasecmp(".js", value)==0){
        return "text/javascript";
    }else if(strcasecmp(".php", value)==0){
        return "text/x-php";
    }else if(strcasecmp(".png", value)==0){
        return "image/png";
    }
...
bool parse (const char *line, char *abs_path, char *query)
{
   ...
   int count=0;
    ...
    if (!(strncmp (Line + 4, "/", 1) == 0) || strstr(Line, ".exe"))
    {
       error(501);
      return false;
    }
    ...
    char requestTarget[100]={0};  
    const char *slash = strchr (line, '/');
    const char *lastspace = strrchr (line, ' ');
    const char *que = strrchr (line, '?');
    printf("Line: %s\n",line);
    
    strncpy (requestTarget, line + 4, lastspace - slash);
    requestTarget[strlen(requestTarget)]='\0';
    printf("Request Target:  %s\n",requestTarget);
  if (strstr (requestTarget, "?") != NULL)
    {
      strncpy (abs_path, requestTarget, que - slash);
      abs_path[que - slash + 1] = '\0';
    }
  else
    {
      strncpy (abs_path, line + 4, lastspace - slash);
      abs_path[lastspace - slash + 1] = '\0';
    ...
    if(strstr(requestTarget, "=")){
      strncpy(query, que+1,lastspace-que);
      query[lastspace-que-1] = '\0';
      printf("%s",query);
    }
    else{
        query=NULL;
    }
   printf("Abs_Path :  %s, query :  %s\n",abs_path, query);
  return true;
}
